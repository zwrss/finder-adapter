...
class JsonHttpClient(client: AsyncHttpClient) {
  ...
  protected def getF(url: String, queryParameters: Map[String, Seq[String]] = Map.empty, cookies: Seq[Cookie] = Seq.empty): Future[JsValue] = {
  ...
  def get(url: String, queryParameters: Map[String, Seq[String]] = Map.empty, cookies: Seq[Cookie] = Seq.empty): JsValue = {
